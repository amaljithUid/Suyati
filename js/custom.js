...
$(window).load(function() {
    ...
    $('.featured-block').equalizeHeights();;
    ...
    $(".side-enelarger").css("right", -sideWidth / 2 - 25).css("width", sideWidth / 2 + 25);
    $(".close-detail").click(function() {
        $(".member-block").removeClass('active');
    });
    $("form.search").hover(function() {
        $('.search-wrapper').stop(true, false).animate({
            left: "-207px",
            width: "300px"
        });
    }, function() {
    ...
    $(".video-block").hover(function() {
        ...
        $(this).find(".tag-area").stop(true, false).animate({
            "bottom": "-30px"
        }, {
            queue: false,
            duration: 200
    ...
    if ($(window).width() > 767) {
        ...
        if ($('.nav-div').length > 0) $('.nav-div').each(function() {
...
$(window).resize(function() {
    changeAnimation();
    $('.splitted-container > .row > div').css('height', 'auto');
    $('.splitted-container > .row > div .text-wrapper').css('height', 'auto');
    if ($(window).width() > 767) {
        if ($('.splitted-container > .row > div:first-child').height() > $('.splitted-container > .row > div:nth-child(2)').height()) {
            $('.splitted-container > .row > div:nth-child(2)').height($('.splitted-container > .row > div:first-child').height());
        } else {
...
$(document).ready(function() {
    ...
    $(".cms-wrapper div.cms-row").last().addClass("last-child");
    ...
    $("#featured-carousel,#search-results-carousel").owlCarousel({
        items: 4,
        navigation: true,
    ...
    $(".client-word-slider").owlCarousel({
        singleItem: true,
        pagination: true
    });
    ...
    $(".job-list .list-wrapper ").mCustomScrollbar({
        setHeight: 190
    });
    wheel();
    var evhighestBox = 0;
    $('.event-block').each(function() {
        if ($(this).height() > evhighestBox) {
            evhighestBox = $(this).height();
        }
    });
    $('.event-block').height(evhighestBox);
	/*$(".partners-tab-div").mCustomScrollbar({
...
$(window).scroll(function(e) {
    ...
    if ($(window).width() > 768) {
        $(".intro-container").css({
            'background-position': '100% ' + (270 - (scrolled * 0.2)) + 'px'
        });
        ...
        $(".intro-container > .img-wrapper").css({
            bottom: (0 + (scrolled) * 0.4)
        });
    }
});
$(document).ready(function() {
    $('#event').change(function() {
        var value = $("#event").val();
        $('#es_txt_group_pg').val(value);
    });
});
$(document).ready(function() {
    $('.newsletter-widget').find("input[type=text], input[type=text], text").each(function(ev) {
        if (!$(this).val()) {
            $(this).attr("placeholder", "Enter Your Email Address");
        }
    });
    $('input[name$="mc_mv_FNAME"]').attr('placeholder', 'Enter Your Name ');
    $('input[name$="mc_mv_EMAIL"]').attr('placeholder', 'Enter Your Email Address');
});
$(document).ready(function() {
    setTimeout('$(".js__p_starts").simplePopup1();', 60000);
    $(".mc_interests_header").html("Get Updates On Category");
    ...
    $(document).scroll(function() {
        ...
        if (y >= height) {
            $('#livesite_engage_button a.ls-engage-button').hide();
        } else {
            $('#livesite_engage_button a.ls-engage-button').show();
        }
    });
});